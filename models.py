...
class StochasticDurationPredictor(nn.Module):
    ...
    def forward(self, x, x_mask, g=None, noise_scale=1.0):
        x = torch.detach(x)  # [1, 192, xx]
        x = self.pre(x)  # [1, 192, xx]
        ...
        z_ = torch.randn(x.size(0), 2, x.size(2)).to(device=x.device, dtype=x.dtype)
        z = z_ * noise_scale
...
class DurationPredictor(nn.Module):
    def __init__(
        ...
        self.drop = nn.Dropout(p_dropout)
        self.conv_1 = nn.Conv1d(
            in_channels, filter_channels, kernel_size, padding=int(kernel_size // 2)
        )
        self.norm_1 = modules.LayerNorm(filter_channels)
        self.conv_2 = nn.Conv1d(
            filter_channels, filter_channels, kernel_size, padding=int(kernel_size // 2)
        )
        self.norm_2 = modules.LayerNorm(filter_channels)
...
class TextEncoder(nn.Module):
    ...
    def forward(
        ...
        x_mask = torch.unsqueeze(commons.sequence_mask(x_lengths, t), 1).to(
            x.dtype
        )
